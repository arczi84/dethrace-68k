...
tAudioBackend_error_code AudioBackend_PlaySample(void* type_struct_sample, int channels, void* data, int size, int rate, int loop, int sample_size) {
    ...
    LOG_TRACE3(" %s channel %d channels %d data %p size %d rate %d loop %d sample_size %d\n", __FUNCTION__, channel, channels, data, size, rate, loop, sample_size);
    ...
    LOG_TRACE3(" %s:%ld ChannelPlaying[%ld] = %ld\n", __FUNCTION__, __LINE__, channel, ChannelPlaying[channel]);
...
tAudioBackend_error_code AudioBackend_PlayCDA(int track) {
    if (!CD_Active) {
        ...
        LOG_TRACE3(" Playing music file %s\n", trackname);
        // Odtwórz sample
        if (AudioBackend_PlaySample(music_sample, channels, audio_data, file_size, sample_rate, 2, sample_size) != eAB_success) {
